...
const StyleInspirations: React.FC<StyleInspirationsProps> = ({ theme }) => {
  ...
  const getOverlayColor = () => {
    switch(theme) {
      ...
      case 'lavender': return 'bg-purple-300';
      ...
      <div className="container mx-auto px-6 max-w-7xl">
        <div className="flex flex-col md:flex-row items-end justify-between mb-20 gap-8">
          <div className="max-w-2xl">
            <span className={`text-[10px] font-black uppercase tracking-[0.5em] mb-4 block transition-colors duration-500 ${getHeaderColor()}`}>Guia de Estilo</span>
          ...
          {STYLE_TIPS.map((item, index) => (
            <div key={index} className="group cursor-default">
              <div className="relative aspect-[3/4] rounded-[3rem] overflow-hidden mb-8 shadow-xl">
                ...
                />
                <div className={`absolute inset-0 opacity-20 transition-all duration-700 group-hover:opacity-40 ${getOverlayColor()}`}></div>
              ...
              <div className={`w-12 h-1 rounded-full transition-all duration-500 group-hover:w-full ${getBarColor()}`}></div>
            </div>
          ))}
        </div>
      </div>
